			...
			function search() {
				...
				function sendCounty(array) {
					...
					var $block = $('#abgne-block-20110317'), $slides = $block
							...
							.find('li'), $control = $block
							.find('.abgne-control'), _animateSpeed = 600,
					// 加入計時器, 輪播時間及控制開關
					...
					for (var i = 0, j = $li.length; i < j; i++) {
						...
						_str += '<li style="list-style-type:none" class="abgne-player-control_'
								+ (i + 1) + '">' + (i + 1) + '</li>';
					}

					// 產生 ul 並把 li 選項加到其中
					var $number = $(
							'<ul style="list-style-type:none"  class="numbers"></ul>')
							.html(_str).appendTo($control), $numberLi = $number
							.find('li');
					...
					$control
							...
							.click(
									function() {
										var _index = $numberLi.filter(
												'.current').index();
										$numberLi
												.eq(
														(this.className
																.indexOf('next') > -1 ? _index + 1
																: _index
																		- 1
																		+ $numberLi.length)
																% $numberLi.length)
